...
sudoku.generate = function (difficulty, unique) {
  ...
  for (var si in shuffled_squares) {
    ...
    if (
      ...
      // Double check board is solvable and, by default, uniquely solvable.
      var solution_count = sudoku._count_solutions(board, 2)
      if (solution_count === 1 || (!unique && solution_count > 0)) {
        return board
      }
    }
  }
...
sudoku._count_solutions = function (board, limit) {
  /* Count number of puzzle solutions up to `limit`.
  ...
  return sudoku._search_count(candidates, limit)
}

sudoku._search_count = function (candidates, limit) {
  /* Count number of solutions represented by `candidates` up to `limit`. */
  if (!candidates) {
    return 0
  }
  if (limit <= 0) {
  ...
  // If every square is assigned, this is one valid solution.
  var max_nr_candidates = 0
  for (var si in SQUARES) {
    var square = SQUARES[si]
    var nr_candidates = candidates[square].length
  ...
  // Branch on the square with the fewest candidates > 1.
  var min_nr_candidates = 10
  var min_candidates_square = null
  for (si in SQUARES) {
    square = SQUARES[si]
  ...
  for (var vi in min_candidates) {
    var val = min_candidates[vi]
    var candidates_copy = JSON.parse(JSON.stringify(candidates))
    var next_candidates = sudoku._assign(candidates_copy, min_candidates_square, val)
    solution_count += sudoku._search_count(next_candidates, limit - solution_count)
